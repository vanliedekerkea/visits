...
public class CustomerController {
    ...
    @Autowired
    public CustomerController(ClinicService clinicService) {
    ...
    public String processFindForm(Customer customer, BindingResult result, Map<String, Object> model) {
        ...
        Collection<Customer> results = this.clinicService.findCustomerByLastName(customer.getLastName());
        if (results.size() < 1) {
            // no customers found
    ...
    @RequestMapping(value = "/customers/{customerId}/edit", method = RequestMethod.PUT)
    ...
    public ModelAndView showCustomer(@PathVariable("customerId") int customerId) {
        ...
        mav.addObject(this.clinicService.findCustomerById(customerId));
        return mav;
    }
